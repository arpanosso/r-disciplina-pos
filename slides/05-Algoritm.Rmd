...
Muitas vezes nos deparamos com situações onde são necessários duas ou mais operações condicionais para classificação. Nessa situações poderemos utilizar a estrutura **Selecione caso**, que no R pode ser acessada com a função `case_when` (do pacote `dplyr`). Essa função é útil para vetorizar instruções condicionais. Isso é semelhante a `if_else` mas pode gerar qualquer número de valores, em vez de apenas `TRUE` ou `FALSE`. Aqui está um exemplo que retorna o dia da semana em função do número de 1 a 7, onde qualquer valor fora desse intervalo seja classificado como `número inválido`. 
